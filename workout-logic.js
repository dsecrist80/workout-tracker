...
window.getWeeklySummary = (workouts) => {
    ...
    const totalVolume = thisWeek.reduce((sum, w) => 
        sum + w.sets.reduce((s, set) => s + (set.w * set.r), 0), 0
    );
    
    // Unique muscles worked
    const musclesWorked = new Set();
    thisWeek.forEach(w => {
        (w.prim || []).forEach(m => musclesWorked.add(m));
    });
    
    // Total sets
    const totalSets = thisWeek.reduce((sum, w) => sum + w.sets.length, 0);
    
    // Volume per muscle
    const volumePerMuscle = {};
    thisWeek.forEach(w => {
        const exerciseVolume = w.sets.reduce((sum, set) => sum + (set.w * set.r), 0);
        (w.prim || []).forEach(m => {
            volumePerMuscle[m] = (volumePerMuscle[m] || 0) + exerciseVolume;
        });
    });
    ...
    return {
        ...
        volumePerMuscle,
        exercises: thisWeek.length
    };
};

// Chart Data for Volume Trends (last 4 weeks)
window.getVolumeTrends = (workouts) => {
    const weeks = [];
    for (let i = 3; i >= 0; i--) {
        ...
        weeks.push({
            week: `Week ${4-i}`,
            volume: volume,
            sets: weekWorkouts.reduce((sum, w) => sum + w.sets.length, 0)
        });
    }
